...
const Login = () => {
  ...
  const { login, loginWithGoogle, setUserType: updateUserType } = useAuth()
  const navigate = useNavigate()

  const hasGoogleAuth = !!import.meta.env.VITE_GOOGLE_CLIENT_ID

  // Google OAuth: first-time user type selection
  ...
  const handleGoogleLogin = async (accessToken) => {
    setError('')
    try {
      const data = await loginWithGoogle(accessToken, 'login')
