pipeline {
    ...
    stages {
        ...
        stage('Build & Deploy') {
            steps {
                // Build and deploy containers. Model training only happens on first deploy
                // (when no model exists). Use Admin Panel retrain button for updates.
                ...
                bat 'docker-compose build --no-cache ml-service'
